...
class Normalizing_characters:
    ...
    def Normalizer_text(self, input_text):
        ...
        if normalized_text.startswith('مساله'):
            normalized_text = re.sub('مساله', ' ', normalized_text)

        return normalized_text.strip()
#mc = Normalizing_characters()
#print(mc.Normalizer_text(' «مسأله ۲۹: آب مضاف هر قدر زیاد باشد، اگر ذره‌ای نجاست به آن برسد،نجس می‌شود.»'))
